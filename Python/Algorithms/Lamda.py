...
def lamda(ex_level,it_num,data,mad,gad):
    ...
    for t in range(0,it_num):
        for j in range(0,num_datos):
            ...
            if ind_gad == 0:
                ...
                num_classes += 1
                conteo.append(1)
                mean_class = np.concatenate((mean_class,(mean_class[0,:]+data[j,:])[None,:]/2),axis = 0)
                std_class = np.concatenate((std_class,mov_std(mean_class[0,:], (mean_class[0,:]+data[j,:])[None,:]/2, 2, std_class[0,:])),axis = 0)
                
            else:
                #print('Newclass')
                conteo[ind_gad] += 1
                new_mean = mean_class[ind_gad,:] + (1/conteo[ind_gad])*(data[j,:]-mean_class[ind_gad,:])
                std_class[ind_gad,:] = mov_std(mean_class[ind_gad,:], new_mean, conteo[ind_gad], std_class[ind_gad,:])
                mean_class[ind_gad,:] = new_mean
                
    gadso = np.zeros((num_classes-1,num_datos))
    for j in range(0,num_datos):
            mads = mad_functions[mad](data[j,:],mean_class[1:,:],std_class[1:,:])
...
def mov_std(old_mean,new_mean,n,old_std):
